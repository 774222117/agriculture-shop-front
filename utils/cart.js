...
function initCartInfo(cartTypeName) {
  ...
  for (let i = 0; i < count; i++) {
    if (cartList[i].name == cartTypeName) {
      isExists = true
      cartInfo = cartList[i]
      break;
    }
  ...
  app.globalData.cartList.push(cartInfo)
  return cartInfo
}
...
function setShopCart(carts, cartTypeName) {
  ...
  if (!isExists) {
    ...
    app.globalData.cartList.push(shopCartInfo)
  }
  //计算购物车合计数据
...
function setShopCartTotal(cartTotal, cartTypeName) {
  ...
  for (let i = 0; i < count; i++) {
    if (cartList[i].name == cartTypeName) {
      cartList[i].cartTotal = cartTotal
      isExists = true
      break
    }
  }
...
function caulTotal(cartTypeName) {
  ...
  let carts = getShopCart(cartTypeName)
  carts.forEach((item) => {
    if (item.realPrice && item.isChoosed) {
      sumPrice += item.realPrice
    } else if(item.isChoosed) {
      sumPrice += item.price
    }
    if (item.realProductPrice && item.isChoosed) {
...
function addShopCart(goods, cartTypeName, isGift) {
  ...
  //计算价格
  cart = caulCartRealPrice(goods, cart)
  //原价金额
 ...
 * @param {商品信息} goods 
 */
function caulCartRealPrice(goods, cart) {
  //计算价格
  cart.realPrice = cart.total * cart.price
  cart.realPrice = parseFloat(cart.realPrice.toFixed(2))
...
function refreshCartNum() {
  let totalBuyCount = getTotalBuyCount()
  if(totalBuyCount>0){
    wx.setTabBarBadge({
      index: 2,
      text: totalBuyCount.toString(),
    })
  }else{
